...
export default function ChangeOwnerModal() {
  ...
  const { mutate: changeOwner, isPending } = useChangeOwner(projectId);
  ...
  return (
    <Dialog open={isChangeOwnerOpen} onOpenChange={closeChangeOwner}>
      <DialogContent className="max-w-md border-none text-muted-foreground">
        ...
        />
        <ScrollArea className="h-40 border border-gray-500 rounded-md p-2">
          {isLoading ? (
            <p>Loading members...</p>
            ...
            filteredMembers.map((member) => (
              <div
                ...
                className={`cursor-pointer p-2 rounded-md ${
                  selectedUser === member.userId._id ? "bg-blue-500 text-white" : "hover:bg-gray-800"
                }`}
                onClick={() => setSelectedUser(member.userId._id)}
              >
                {member.userId.name}
              </div>
            ))
