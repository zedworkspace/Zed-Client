...
const page = () => {
  ...
  const { data: repo, isSuccess } = useRepo(repoName, isEnable);
  const { data: commits } = useRepoCommit(repoName, brach, isEnable);
  const { data: contributors } = useRepoContributors(repoName, isEnable);
  const { data: pull } = useRepoPulls(repoName, isEnable);
  const { data: issues } = useRepoIssues(repoName, isEnable);
  const { data: branches } = useRepoBranches(repoName, isEnable);
  console.log(commits);
  console.log(repo);
  ...
  const branches1 = [{ name: "main" }, { name: "beta" }];

  useEffect(()=>{
    qureyClient.invalidateQueries({queryKey:['commit',repoName]})
  },[brach])
                  ...
                  <div className="flex gap-5 items-center">
                    ...
                    <div>
                      <div className="flex gap-3 items-center">
                        ...
                        <span className="text-gray-400 text-xs">{moment(commits?.author?.date).fromNow()}</span>
                      </div>
                      <div>
                        <p className="text-sx">{commit.commit.message}</p>
                      </div>
                    </div>
                  </div>
                );
                    ...
                    <div className=" w-full flex flex-col gap-3">
                      ...
                      <div className="flex gap-3 items-center">
                        ...
                        <div className="flex flex-col">
                          <p className=" text-gray-400">{repo?.owner?.login}</p>
                  ...
                  <div className="bott flex justify-between">
                    <div className="flex gap-2 text-gray-400 items-center">
                      ...
                      <p className="text-sm">
                        ...
                        {moment(commits?.[commits.length - 1]?.author.date).fromNow()} 
